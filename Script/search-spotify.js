...
if (inputSearchVal.style.display = 'block') {
    ...
    const APIController = (function () {
        ...
        const _searchTracks = async (token, searchKeyword) => {
            ...
            const searchResult = await fetch(`https://api.spotify.com/v1/search?q=${searchKeyword}&type=track%2Cartist`, {
                method: 'GET',
                headers: { 'Authorization': 'Bearer ' + token }
            });
    ...
    const UIController = (function () {
        ...
        return {
            ...
            createTrack(id, name, artist) {
                ...
                document.querySelector(DOMElements.matchList).insertAdjacentHTML('beforeend', html);
            },

            // create track detail on the left
            createTrackDetail(img, title, artist) {
                
                const insertIMGFrom = document.querySelector(DOMElements.imageCoverWrapper);
                const insertSongTitleFrom = document.querySelector(DOMElements.insertSongTitle);
                const insertArtistFrom = document.querySelector(DOMElements.insertArtistName);
